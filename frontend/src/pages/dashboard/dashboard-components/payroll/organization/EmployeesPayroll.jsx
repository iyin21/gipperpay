...
const EmployeesPayroll = () => {
  ...
  return (
    <div className="px-8">
      <div>
        <div className="lg:flex justify-between  items-center h-auto">
          ...
          <div className="w-[65%] flex justify-between ">
            ...
            <div className="bg-white-80 flex h-auto items-center w-[30%] rounded-lg  p-2 gap-4 flex-end">
        ...
        <div className="block lg:flex mt-4 justify-between h-auto">
          <div>
            <p className="text-white-30 text-s font-regular leading-5">Organization balance</p>
            <p className="text-secondary-50 font-medium text-sm">$0.00</p>
          </div>
          <button className=" w-full lg:w-[12%] h-10 bg-primary-main rounded-md text-whiteText text-s font-regular leading-6">Top Up Balance</button>
        </div>
        <div className=" block lg:flex justify-between h-auto items-center my-4">
          <p className="text-white-30 text-s font-regular leading-5">Next Payment Date</p>
          <div>
                  <input
                    name="startDate"
                    type="date"
        ...
        <div className="flex justify-between h-auto items-center my-3">
          <p className="text-white-30 text-s font-regular leading-5">Amount due</p>
          <p className="text-secondary-50 text-s font-medium">$49,000</p>
          <div className="flex h-auto items-center gap-3">
          <span className="text-white-30 text-s font-regular leading-5">Auto</span>
          <label className="relative inline-flex items-center mr-5 cursor-pointer">
            
            <input type="checkbox" value="" className="sr-only peer bg-white-70" checked />
           
            <div className="w-11 h-6 bg-gray-200 rounded-full peer dark:bg-gray-700 peer-focus:ring-4 peer-focus:ring-green-300 dark:peer-focus:ring-green-800 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-green-600"></div>
          </label>
          </div>
        </div>
        <div className=" lg:flex h-auto items-center block justify-between  m-auto w-full">
          <p className="flex gap-4 w-[15%]"> 
            <img src={PayrollLogo} alt='payroll' className="w-[20px] h-[20px]"/>
            <span className="text-secondary-50 text-s font-medium">Payroll</span></p>
          <form className="flex items-center w-[50%]">
            <div className="relative w-[100%]">
              <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <AiOutlineSearch color="#7B7B7B"/>
              </div>
              <input type="text" className=" text-white-30 pl-10 w-full outline-none h-[2.8rem] px-5 leading-5 font-regular bg-white-70 rounded-sm" placeholder='Search'/>
            </div>
          </form>
          <p className="flex gap-4 w-[20%] cursor-pointer" onClick={handleModal}>
      ...
      <PayrollTable/>
      {openModal && <PayrollLinkModal handleModal={handleModal} openModal={openModal}/>}
    </div>
  );
};
